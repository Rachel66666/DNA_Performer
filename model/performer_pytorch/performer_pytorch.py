...
from helpers import model_summary, model_summary2
...
class FastAttention(nn.Module):
    def __init__(self, dim_heads, nb_features = None, ortho_scaling = 0, causal = False, generalized_attention = False, kernel_fn = nn.ReLU(), no_projection = False):
        ...
        print("projection is ", str(no_projection))
...
class FeedForward(nn.Module):
    def __init__(self, dim, mult = 1, dropout = 0., activation = None, glu = False):
        super().__init__()
...
class Performer(nn.Module):
    def __init__(
        ...
        for _, local_heads in zip(range(depth), local_attn_heads):
            ...
            feedf = FeedForward(dim, mult = ff_mult, dropout = ff_dropout, glu = ff_glu)
            ff = Chunk(ff_chunks, feedf, along_dim = 1)

            # print("FeedForward: ")
            # model_summary2(feedf)
